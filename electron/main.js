...
app.whenReady().then(() => {
  ...
  ipcMain.handle("db:addProject", async (_e, name, color) => db.addProject(name, color));
  ipcMain.handle("db:updateProject", async (_e, id, patch) => db.updateProject(id, patch));
  ipcMain.handle("db:deleteProject", async (_e, id) => db.deleteProject(id));
